"""Codon adaptation analysis (DIAMOND + pre-built CSDB reference)."""
...
def write_codon_adaptation_results_tsv(
        ...
        *,
        cai_engineered_threshold: float,
) -> str:
...
def analyze_codon_adaptation(
    ...
    """
    DIAMOND blastx + CSDB reference: ORF coordinates from qstart/qend, host taxid from staxids, CAI.

    Set ``debug_write_diamond_output`` + ``diamond_output_path`` to save TSV for reuse.
    Set ``run_diamond=False`` + ``diamond_output_path`` to load a saved DIAMOND TSV.
    """
...
def write_codon_adaptation_tsv(
    ...
    """Run codon adaptation and write TSV; returns (output path, results)."""
