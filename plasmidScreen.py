...
def screen(ctx: typer.Context, fasta_file: Annotated[str, typer.Argument(help="Fasta file ")],
           ...
           kraken_output_path: Annotated[str | None, typer.Option("--kraken-output-path",
                                                                 help="Optional path to read/write raw Kraken2 classifications. Required when --run-kraken=false or when --debug-write-kraken-out is set.")] = None,
           ...
           diamond_db: Annotated[str | None, typer.Option("--diamond-db",
                                                         help="DIAMOND database (.dmnd) used to infer ORFs/host taxid for codon CAI. Required when codon usage is enabled.")] = None,
           diamond_threads: Annotated[int, typer.Option("--diamond-threads",
                                                       help="Threads for DIAMOND (defaults to --threads if not set).")] = 0,
           diamond_extra_args: Annotated[str | None, typer.Option("--diamond-args",
                                                                 help="Extra DIAMOND args passed verbatim (comma-separated).")] = None,
           skip_codon_usage: Annotated[bool, typer.Option("--skip-codon-usage",
                                                         help="Skip codon adaptation/CAI analysis (engineered k-mer scan only).")] = False,
           debug_write_kraken_out: Annotated[bool, typer.Option("--debug-write-kraken-out",
                                                               help="Write raw Kraken2 classifications to --kraken-output-path (debug).")] = False,
           ...
           run_kraken: Annotated[bool, typer.Option("--run-kraken/--no-run-kraken",
                                                   help="Run Kraken2 as part of the pipeline (default). If disabled, --kraken-output-path must point to an existing classifications file.")] = True,
    ...
    if not skip_codon_usage and not diamond_db:
